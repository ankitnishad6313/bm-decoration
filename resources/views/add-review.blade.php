...
@section('main-section')
    <div class="container">
        ...
        <div class="password-container">
            <h1>Add Review</h1>
            <form action="{{ route('user-review-store') }}" method="post" class="password-form">
                @csrf
                <input type="hidden" name="product_id" value="{{ $product_id }}">
                <div class="input-fields">
                    <label for="star">Star *</label>
                    <input type="text" name="star" class="current-password" value="{{ old('star') }}" id="star" min="1" max="5" required />
                </div>
                <div class="input-fields">
                    <label for="comment">Comment *</label>
                    <textarea name="comment" id="comment" class="current-password" rows="3">{{ old('comment') }}</textarea>
                </div>
